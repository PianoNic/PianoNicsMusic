...
class BassBoostUp(commands.Cog):
    ...
    async def run(self, ctx):
        ...
        await responses.confirm(
            ctx, "🎸", "Bass Boost Up", f"Bass boost increased to {percent}% [{meters.bar(percent, 20)}]"
        )

    @commands.command(aliases=['bass+', 'bassup', 'more_bass'])
    async def bass_boost_up(self, ctx):
        await self.run(ctx)

    @discord.slash_command(name="bass_boost_up", description="Increases bass boost by 10%")
    async def bass_boost_up_slash(self, ctx):
        await self.run(ctx)


def setup(bot):
    bot.add_cog(BassBoostUp(bot))
